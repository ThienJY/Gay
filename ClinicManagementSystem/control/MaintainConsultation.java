...
public class MaintainConsultation {
    ...
    private final ListInterface<Consultation> history = new List<>();
    private final ConsultationUI ui;

    public MaintainConsultation(PatientMaintenance patientMaintenance, DoctorManagementControl doctorManagement) {
        ui = new ConsultationUI(patientMaintenance, doctorManagement);
    }
    ...
    private void enqueueWalkIn() {
        ...
        if (p == null) {
            ui.showMessage("Cannot enqueue: Invalid patient ID.\n");
            return;
        }
        Doctor d = ui.inputDoctor();
        if (d == null) {
            ui.showMessage("Cannot enqueue: Invalid doctor ID.\n");
            return;
        }
    ...
    private void scheduleAppointment() {
        Patient p = ui.inputPatient();
        if (p == null) {
            ui.showMessage("Cannot schedule: Invalid patient ID.\n");
            return;
        }
        Doctor d = ui.inputDoctor();
        if (d == null) {
            ui.showMessage("Cannot schedule: Invalid doctor ID.\n");
            return;
        }
        String cid = ui.inputConsultationId();
    ...
    private String generateReports() {
        ...
        int queueLen = waitingQueue.getSize();
